...
public class Player1 extends JComponent{
	...
	public void draw(Graphics g){
		...
		boolean over = false;
		while (over==false){
			bally = (int) (((ballx-100)*Math.tan(A))-((9.8*Math.pow(ballx-100, 2))/(2*Math.pow((v*Math.cos(A)),2))));
			//calculating the ball height 
			//bally =  (int) (((ballx-100)*Math.tan(A))-((9.8*Math.pow(ballx-100, 2))/(2*Math.pow((v*Math.cos(A)),2))));
			System.out.println(bally);
			//drawing trajectory
			g.setColor(Color.black);
			g.drawLine(ballx, 720-bally, ballx, 720-bally);
		 ...
		 if (ballx>=410){
				...
				//the ball hits the ground
					...
					if(wall.getBricks()[rowIndex][colIndex].getState().equals("normal")){
						...
						//wall.getBricks()[rowIndex][colIndex].setColor(Color.MAGENTA);
						score+=10;
						over = true;
					...
					if (ballx>=1100 && ballx<1200 && bally>=700 && bally<=750){
						enemyhit = true;
